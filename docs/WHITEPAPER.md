...
1.  **Phase 1: The Sovereign Vault (Current):** A high-availability architecture where the Vault acts as the arbiter of truth, verifying PoW difficulty and enforcing the 100B monetary policy.
2.  **Phase 2: Hydra P2P:** Future transition to a fully decentralized gossip protocol. Consensus will be achieved via Byzantine Fault Tolerant (BFT) voting among all active nodes, deprecating the centralized Vault.
